...
cat(emo::ji("white_heavy_check_mark"), length((.packages())), "packages loaded")
```
...
fixHeader <- T              # whether to remove everything after : in each line of the header
...
dayOfChange <- 7            # What experimental day the change occurs on (First day of experiment is Day 1)
...
```{r behavr table}
data <- rbind(C676, C678, C679, C680, C707, C708, C709, C710, C711) %>%  filter(as.integer(expDay)>7 & as.integer(expDay)<30)
#data$MouseName = "C676"
...
ggetho(dt, aes(z=speed), timewrap = behavr::hours(12)) +
      stat_tile_etho() +
      stat_ld_annotations() +
            # on top of it, a second layer that
            # starts at day 7 thoughout day 30,
            # and where L colour is grey
            stat_ld_annotations(x_limits = behavr::days(c(7,30)),
...
ggetho(dt, aes(z = speed), multiplot = 2, multiplot_period = behavr::hours(24))+
  stat_bar_tile_etho()+
  stat_ld_annotations() +
  stat_ld_annotations(x_limits = behavr::days(c(1,2)),
                                ld_colours = c("grey", "black"), l_duration=behavr::hours(5))+
  facet_wrap( ~ Genotype + id, ncol=3, labeller = label_wrap_gen(multi_line=FALSE))
```
