...
```{r Phases, echo=F, fig.height=1, fig.width=5}

# Phases of your experiment. Name the phases of your experiment and the day each phase starts on. 
...
```
### `r emo::ji("calendar")` Add experimental phases

```{r Data phases, include=FALSE}
...
# Dislay approx. experimental timeline (note that the last phase is just set to a constant since the script doesn't know when an experiment ends at this point)
ggplot(head(phase_df,-1), aes(fill=factor(phase, levels=rev(phases[1:length(phases)-1])), x=duration, y="Experimental timeline")) + 
