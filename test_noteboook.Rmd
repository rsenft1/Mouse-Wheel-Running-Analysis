...
```{r 5. Add columns to data, include=FALSE}
#Source functions
...
```{r 6. Make behavr table, results = 'asis', echo=FALSE}
...
```{r 7. Add and display experiment phases, echo=FALSE, fig.height=2, fig.width=6}
#Source functions
...
```{r 8. Save tables of metadata and binned data (OPTIONAL), include=FALSE}
...
```{r 9. Bout analysis, fig.width=8, echo=FALSE}
## bout length ##
...
```{r 10. Average Day graphs, echo=FALSE}
#mean +/- 1 SE
startDay=1
...
ggetho(dt, aes(z=speed), timewrap = behavr::hours(24)) +
      ...
      stat_tile_etho() +
      stat_ld_annotations() +
      stat_ld_annotations(x_limits = behavr::days(c(dayOfChange,endDay-1)),
...
ggetho(dt,aes(z = speed),
      ...
      stat_ld_annotations() +
            ...
            stat_ld_annotations(x_limits = behavr::days(c(1,2)),
                                ld_colours = c("grey", "black"), l_duration=behavr::hours(5), position=position_nudge(x=0, y=-2))+
    scale_y_discrete(labels=rev(seq(startDay, endDay-1)))

    scale_y_continuous(minor_breaks = seq(startDay, endDay, 0.5))
...
for (eachMouse in abbrev_mice){
  ...
  p <- ggetho(dt[xmv(MouseName)==eachMouse,], aes(y=speed, color=id)) +
          stat_ld_annotations(height=1, alpha=0.2, outline = NA, x_limits = behavr::days(c(0,dayOfChange-1))) +
                # on top of it, a second layer that
                # starts at day 7 thoughout day 30,
                # and where L colour is grey
          stat_ld_annotations(height=1, alpha=0.2, outline=NA, x_limits = behavr::days(c(dayOfChange-1,endDay-1)),l_duration=behavr::hours(5))+
          stat_pop_etho() +
...
for (phase in phase_df$phase){
  ...
  # ggperio(per_xsq_dt) + geom_line(aes(group = id, color=Genotype)) # if you want to plot ind. lines.
